...
MUTAZ_RAW_COL = "mutaz"
ALPHA = 0.05  # soglia di significatività sotto cui si generano i grafici
...
def generate_plots(df_gen: pd.DataFrame, results: pd.DataFrame, generation: int, plots_dir: Path) -> pd.DataFrame:
    ...
    sig = results[results["pvalue"].notna() & (results["pvalue"] < ALPHA)].copy()
    sig["plot_path"] = None
...
def build_word_report(results: pd.DataFrame, sig: pd.DataFrame, generation: int, out_path: Path):
    ...
    doc.add_paragraph(
        ...
        f"Risultati significativi (p < {ALPHA}): {results['pvalue'].notna().sum() and (results['pvalue'] < ALPHA).sum()}."
    )

    doc.add_heading("Tabella risultati significativi", level=2)
    table = doc.add_table(rows=1, cols=6)
    table.style = "Light Grid Accent 1"
    hdr = table.rows[0].cells
    for i, h in enumerate(["Variante", "Variabile", "Test", "p-value", "N gruppo 0", "N gruppo 1"]):
        hdr[i].text = h

    for _, row in sig.sort_values("pvalue").iterrows():
...
def build_combined_report(sig1: pd.DataFrame, sig2: pd.DataFrame, out_path: Path):
    from docx import Document
    from docx.shared import Inches
    ...
    doc.add_paragraph(
        f"Coppie (variante, variabile) significative in almeno una generazione: {len(all_keys)}."
    )
    ...
    for variant, var_col in all_keys:
        ...
        p1 = f"{row1['pvalue'].iloc[0]:.4g}" if not row1.empty else "n.s. / non testato"
        p2 = f"{row2['pvalue'].iloc[0]:.4g}" if not row2.empty else "n.s. / non testato"
        doc.add_paragraph(f"Generazione 1: p = {p1}    |    Generazione 2: p = {p2}")
...
def main():
    ...
    if 1 in sig_by_gen and 2 in sig_by_gen:
        build_combined_report(
            sig_by_gen[1], sig_by_gen[2], REPORTS_DIR / "combined_report.docx"
        )
